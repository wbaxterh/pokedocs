...
- **S3.2.4 — Generated HTTP discovery for hosts that allow it**
  *As P4, I want responses from a PokeDocs site to carry `Link` headers to the agent artifacts, and page URLs to return the markdown twin when I send `Accept: text/markdown`, so that an agent gets clean markdown from the URL it already has without knowing the `.md` convention.*
  Acceptance: `pokedocs deploy init` writes the `Link` header for every target that can set response headers: `static/_headers` (Netlify, Cloudflare Pages), `vercel.json` `headers` (Vercel), and `add_header` in the generated nginx conf (Docker); the Docker target also negotiates `Accept: text/markdown` (a `map $http_accept` plus a server-level rewrite that fires only when the twin exists, so assets and `llms.txt` are untouched) and sends `Vary: Accept`; `Link` targets are derived from the same artifact list as S3.2.1 so the two cannot disagree; GitHub Pages, which cannot set headers, is documented as a limitation with the HTML `<link rel>` tags (S1.5.3) as its fallback; the nginx behavior verified with `curl -H 'Accept: text/markdown'` against the real nginx image. Scope note (2026-09-24): static config cannot negotiate on the other hosts (Netlify and Cloudflare redirect conditions cannot match request headers, and Vercel serves an existing file before any rewrite), so that moved to S3.2.5.
- **S3.2.5 — Edge negotiation for serverless hosts**
  *As P4 reading a PokeDocs site on Netlify, Cloudflare Pages, or Vercel, I want `Accept: text/markdown` on a page URL to return its markdown twin, as it already does on the Docker target, so that negotiation does not depend on which host the owner picked.*
  Acceptance: `pokedocs deploy init` writes the host's edge hook alongside S3.2.4's headers: a Netlify Edge Function, a Cloudflare Pages Functions `_middleware`, and Vercel Routing Middleware; each serves the twin only when it exists, sets `Content-Type: text/markdown` and `Vary: Accept`, and re-attaches the `Link` header where the host skips static header rules on function responses (Cloudflare does); HTML requests and assets pass through untouched; each verified with `curl -H 'Accept: text/markdown'` against a real deploy on that host.
